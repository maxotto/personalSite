...
trait UserTrait
{
    ...
    /**
     * Getter for refresh exp that's used for generation of JWT refresh token
     * @return string secret key used to generate JWT
     */
    protected static function getRefreshJwtExpire()
    {
        return Yii::$app->params['JWT_REFRESH_EXPIRE'];
    }
    ...
    public function getRefreshJWT($payload = [])
    {
        $secret = static::getSecretKey();
        $currentTime = time();
        $request = Yii::$app->request;
        ...
        $payload['uid'] = $this->getPayloadUid();
        if (!isset($payload['exp'])) {
            $payload['exp'] = $currentTime + static::getRefreshJwtExpire();
        }
        return JWT::encode($payload, $secret, static::getAlgo());
    }
}
